    ...
    return (
        <Link to={`/pokemon/${name}`} style={{ textDecoration: 'none' }}>
            ...
            >
                ...
                <img src={data.sprite} alt={name} style={{ width: '80px', height: '80px', objectFit: 'contain', filter: 'drop-shadow(0 4px 8px rgba(0,0,0,0.5))', position: 'relative', zIndex: 1 }} />
                <p style={{ color: 'white', fontWeight: 'bold', textTransform: 'capitalize', fontSize: '0.8rem', margin: 0, textAlign: 'center', lineHeight: '1.2' }}>
                    {name.replace(/-/g, ' ')}
                </p>
                <div style={{ display: 'flex', gap: '0.25rem', flexWrap: 'wrap', justifyContent: 'center' }}>
                    {data.types.map(type => (
                        <span key={type} style={{
                            ...
                            fontSize: '0.65rem', fontWeight: 'bold', padding: '0.15rem 0.45rem',
        ...
        <div style={{ marginBottom: '2.5rem' }}>
            <div style={{
                ...
                <div style={{
                    ...
                    backgroundColor: `${category.color}22`, border: `2px solid ${category.color}55`,
                ...
                <div>
                    <h2 style={{ color: 'white', fontWeight: 'bold', fontSize: '1.2rem', margin: 0 }}>
                        ...
                        <span style={{
                            ...
                            fontSize: '0.75rem', fontWeight: 'bold', padding: '0.1rem 0.55rem',
                            borderRadius: '9999px', marginLeft: '0.4rem', verticalAlign: 'middle',
                        }}>{category.pokemon.length}</span>
                    </h2>
                    <p style={{ color: '#6b7280', fontSize: '0.8rem', margin: '0.15rem 0 0' }}>{category.description}</p>
                </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: '0.85rem' }}>
                {category.pokemon.map(name => (
                    <PokemonCard key={name} name={name} categoryColor={category.color} categoryGlow={category.glow} />
                ))}
            </div>
        </div>
    )
    ...
    return (
        <div style={{ maxWidth: '960px', margin: '0 auto', padding: '2rem 1rem 5rem' }}>
            ...
            <PageHeader
                title="Rare Pokémon"
                icon="🌟"
                subtitle={`${totalCount} extraordinary Pokémon across 4 categories`}
                search={search}
                onSearch={setSearch}
                ...
                {filters.map(f => (
                    <button
                        ...
                        style={{
                            ...
                            border: `2px solid ${activeFilter === f.id ? (f.color ?? '#cc2020') : '#374151'}`,
                            borderRadius: '9999px', cursor: 'pointer', fontWeight: 'bold',
                            fontSize: '0.85rem', transition: 'all 0.18s',
                            backgroundColor: activeFilter === f.id ? (f.color ?? '#cc2020') : '#1f2937',
                            color: 'white',
                            boxShadow: activeFilter === f.id ? `0 4px 12px ${f.color ?? '#cc2020'}55` : 'none',
            ...
            {CATEGORIES.map(category => {
                const filteredCategory = search.trim()
                    ...
                    : category
                const isVisible = (activeFilter === 'all' || activeFilter === category.id) && filteredCategory.pokemon.length > 0
                return <CategorySection key={category.id} category={filteredCategory} isVisible={isVisible} />
            })}

            {search.trim() && CATEGORIES.every(c => !c.pokemon.some(n => n.includes(search.toLowerCase().trim()))) && (
