...
// Form overrides — maps display name to PokéAPI endpoint that has sprites
    ...
    if (!data) return (
        <Link to={`/pokemon/${name}`} style={{ textDecoration: 'none' }}>
            <div style={{
                ...
                border: '2px solid transparent', minHeight: '160px', justifyContent: 'center',
            }}>
                <div style={{ width: '96px', height: '96px', borderRadius: '50%', backgroundColor: '#374151', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2rem' }}>?</div>
                <span style={{ color: 'white', fontWeight: 'bold', fontSize: '0.85rem', textTransform: 'capitalize', textAlign: 'center' }}>
                    {name.replace(/-/g, ' ')}
                </span>
            </div>
        </Link>
    ...
    return (
        <div style={{ maxWidth: '960px', margin: '0 auto', padding: '2rem 1rem 5rem' }}>
            ...
            <PageHeader
                ...
                searchValue={search}
                onSearch={setSearch}
                searchPlaceholder="Search rare Pokémon..."
            >
                {/* ── Filter tabs ── */}
                <div style={{
